...
The first store listing must be created manually. The workflow needs these repository secrets: `CHROME_EXTENSION_ID`, `CHROME_CLIENT_ID`, `CHROME_CLIENT_SECRET`, `CHROME_REFRESH_TOKEN` (create them with `bunx wxt submit init`), plus `BACKEND_DISPATCH_TOKEN`, a fine-grained token with Contents read and write access to `storylens-backend`, which the repository dispatch API requires. The SSH deploy secrets live in the backend repository.
